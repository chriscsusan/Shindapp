...
<body>
	<div class="background">
		<div class="transbox">
			...
			<h3>
				<form method="POST"
					action="<c:url value="/item-info/${id}/delete"/>">
					<c:forEach var="ingredient" items="${ingredients}">
						<li><c:out value="${ ingredient.name }" /> <c:if
								test="${showAll=='true'}">
								<button name="foodName" type="submit"
									value="${ ingredient.name }">Delete</button>
							</c:if> <input type="hidden" name="id" value="${id}" />
					</c:forEach>
				</form>
			</h3>
	...
	<nav>
		<a href="<c:url value="/"/>">HOME</a> <a
			href="<c:url value="/sign-up"/>">SIGN UP</a>
	</nav>
	<div>
	...
	<c:if test="${showAll!='true'}">
		<div>
			<form method="post">
				<h2>
					Is this your dish? Enter pin: <input type="text" name="pin" />
					<button type="submit">Submit</button>
				</h2>
				<input type="hidden" name="id" value="${id}" /> <input
					type="hidden" name="firstName" value="${signupEntry.firstName}" />
				<input type="hidden" name="lastName" value="${signupEntry.lastName}" />
				<input type="hidden" name="phoneNumber"
					value="${signupEntry.phoneNumber}" /> <input type="hidden"
					name="dishName" value="${signupEntry.dishName}" />
			</form>
		</div>
	</c:if>
	...
	<c:if test="${showAll=='true'}">
		...
		<h1>Ingredient Search: Make a list of your dish's ingredients one
			by one to identify if it contains an allegen</h1>
